<x-app-layout>
    ...
    <script>
        $(document).ready(function() {
            ...
            @foreach($reportstockspg->details as $index => $detail)
                const existingRow = $('#row-{{ $index + 1 }}');
                const itemCode = '{{ $detail->item_code }}';
                const itemName = '{{ $detail->nama_barang }}';
                const ukuran = '{{ $detail->ukuran }}';
                
                // Initialize readonly select untuk item existing
                existingRow.find('.item-select').select2({
                    data: [{
                        id: itemCode,
                    ...
                    width: '100%',
                    dropdownParent: existingRow.closest('.table-container'),
                    disabled: true
                }).val(itemCode).trigger('change');
            @endforeach
            
            // Tombol tambah item baru
            $('#addItemBtn').on('click', function(e) {  // Tambahkan parameter e di sini
                e.preventDefault();  // Pindahkan ke sini
