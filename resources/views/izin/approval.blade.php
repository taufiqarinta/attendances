<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <div class="mb-4 flex justify-end">
                        <input type="hidden" id="approve_all_data" value="">
                        <button id="btnApproveAll" disabled onclick="modalApproveAll()" 
                            class="bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white font-bold py-2 px-4 rounded flex items-center gap-2">
                    ...
                    <div class="table-container w-full overflow-hidden">
                        <div class="overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300">
                            <table class="w-full divide-y divide-gray-200" id="tblapproval">
    ...
    <script>
        ...
        const approveAllData = document.getElementById('approve_all_data');
        ...
        async function loadApprovalData() {
            try {
                ...
                if (result.success) {
                    allData = result.data || [];
                    renderTable(allData);
                    showAlert('success', 'Data approval berhasil dimuat');
                } else {
        ...
        function filterData() {
            ...
            const filtered = allData.filter(item => {
                const itemDateStr = item.created_date || item.crtd;
        ...
        function renderTable(data) {
            ...
            data.forEach((item, index) => {
                ...
                if (item.status === 0) {
                    ...
                    } else if (item.approval === 1 && (item.approval2 === 0 || item.approval2 === null)) {
                        badgeClass = 'bg-yellow-500';
                ...
                if (!disableApprove) {
                    actionHtml = `
                        <button class="bg-blue-500 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm approve-btn" 
                            data-personnelno="${item.personnelno}" 
                            data-startdate="${item.sd}" 
                            data-enddate="${item.ed}" 
                            data-keterangan="${item.keterangan}" 
                            data-tipe="${item.kode_tipe}"
                ...
                html += `
                    <tr>
                        ...
                        <td class="px-4 py-3 text-center whitespace-nowrap">
                            <span class="${badgeClass} text-white px-2 py-1 rounded-full text-xs whitespace-nowrap">
                                ${statusText}
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            ${!disableApprove ? `
                                <input type="checkbox" class="checkbox-item rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" 
                                    data-personnelno="${item.personnelno}"
                                    data-startdate="${item.sd}"
                                    data-enddate="${item.ed}"
                                    data-keterangan="${item.keterangan}"
        ...
        function renderEmptyTable() {
            // Buat baris dengan jumlah kolom yang sesuai (10 kolom)
            let emptyRow = '<tr>';
            for (let i = 0; i < 10; i++) { // Loop sebanyak 10 kali (jumlah kolom)
                emptyRow += '<td class="px-6 py-4 text-center text-gray-500"></td>';
            }
            emptyRow += '</tr>';
            
            // Kemudian timpa dengan pesan "Tidak ada data" yang menggunakan colspan
            // tapi pastikan DataTables sudah selesai inisialisasi
            ...
            setTimeout(() => {
                ...
                // Setelah DataTable terinisialisasi, baru tampilkan pesan colspan
                $('#tblapproval tbody').html(`
                    <tr>
        ...
        function attachEventListeners() {
            ...
            document.querySelectorAll('.approve-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    const data = {
                        personnelno: this.dataset.personnelno,
        ...
        // Update selected items for approve all
        function updateSelectedItems() {
            ...
            checkboxes.forEach(cb => {
                selectedItems.push({
                    nik: cb.dataset.personnelno,
                    start_date: cb.dataset.startdate,
                    end_date: cb.dataset.enddate,
            ...
            // Update hidden input
            approveAllData.value = JSON.stringify(selectedItems);
            ...
            // Update check all
            const totalCheckboxes = document.querySelectorAll('.checkbox-item').length;
        ...
        function showApprovalDialog(data) {
            Swal.fire({
                ...
                text: `Apakah Anda yakin untuk approve pengajuan ini?`,
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    // Approve
                    submitApproval(data, 1);
                } else if (result.isDenied) {
                    // Reject with reason
                    showRejectDialog(data);
                }
            });
        }

        // Show reject dialog
        function showRejectDialog(data) {
            Swal.fire({
                title: 'Alasan Reject',
                input: 'textarea',
                ...
                inputAttributes: {
                    'aria-label': 'Tulis alasan reject di sini'
                },
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    submitApproval(data, 0);
                }
            });
        }

        // Submit approval to API
        async function submitApproval(data, isapprove) {
            try {
                showLoading();
                
                const formData = new FormData();
                formData.append('personnelno', data.personnelno);
                formData.append('atasan', USER_NIK);
        ...
        // Modal approve all
        function modalApproveAll() {
            if (selectedItems.length === 0) {
                Swal.fire({
                    icon: 'warning',
                    title: 'Peringatan!',
                    text: 'Pilih data yang akan di-approve terlebih dahulu'
                });
                return;
            }
            
            Swal.fire({
                title: 'Konfirmasi Approve All',
                text: `Apakah Anda yakin untuk approve ${selectedItems.length} data yang dipilih?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonText: 'Ya, Approve All!',
                cancelButtonText: 'Batal',
                confirmButtonColor: '#28a745'
            }).then((result) => {
                if (result.isConfirmed) {
        ...
        async function submitApproveAll() {
            try {
                ...
                const formData = new FormData();
                formData.append('bulk_data', approveAllData.value);
                formData.append('atasan', USER_NIK);
                
                const response = await fetch(API_APPROVE_ALL_URL, {
                    method: 'POST',
                    body: formData
                });
